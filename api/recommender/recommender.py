            ...
            for j in range(len(Ratings[i])):
                ...
                if Ratings[i][j] > 0:
                    ...
                    for w in range(features):
                        UserFeatures[i][w] += (2 * learningRate * error *
                                               ItemFeatures[w][j] - invariant * UserFeatures[i][w])
                        ItemFeatures[w][j] += (2 * learningRate * error *
                                               UserFeatures[i][w] - invariant * ItemFeatures[w][j])
        ...
        for i in range(len(Ratings)):
            for j in range(len(Ratings[i])):
                ...
                if Ratings[i][j] > 0:
                    ...
                    MSE += pow(Ratings[i][j] -
                               dot(UserFeatures[i, :], ItemFeatures[:, j]), 2)

                    # Regularization
                    for w in range(features):
                        MSE += normaliz * \
                            (pow(UserFeatures[i][w], 2) +
                             pow(ItemFeatures[w][j], 2))
...
for user in userIDs:
    ...
    myPersonalRecommendations = PredictedRatings[userIDs[user], :].argsort()[
        ::-1][:top]
